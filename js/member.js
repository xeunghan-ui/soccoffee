...
function capPreview(info, p){
  ...
  const used = (info.counts && info.counts[g] ? info.counts[g].used : 0);   // 유지 확인 인원
  const retApplied = ((info.retQueue || {})[g] || []).length;
  return { g, cap, used, left: Math.max(0, cap - used),
           rank: retApplied + 1 };   // 지금 신청하면 받을 복귀 순번
}
...
function renderCapAsk(m, dorm, pv){
  ...
  const body = dorm
    ? line(`복귀는 <b style="color:#ece6d2">순번제</b>예요 — 지금 신청하면 ${gl} <b style="color:var(--accent)">${pv.rank}번</b>입니다.`)
      + line(`26일에 유지 확인이 끝난 뒤 <b style="color:#ece6d2">남은 자리를 순번대로 배정</b>해요. 신청이 자리를 미리 차지하지는 않습니다.`)
...
async function capSetNext(memberId, month, dormant){
  ...
  if (!dormant && meP && isDormantFor(meP, nowMonthStr())) {
    // 복귀 신청 = 순번만 부여, 자리는 26일 확정 때 순번대로 배정(팀빌더 활동 예외는 기록하지 않음)
    let info; try { await loadCapConfirm(month); info = capCompute(month, await fetchDues(month)); } catch(e){ info = null; }
    const rn = info && info.retRank ? info.retRank[memberId] : null;
    toast(rn ? ('복귀 순번 ' + rn + '번으로 신청했어요 — 26일에 남은 자리를 순번대로 배정해요') : '복귀 신청을 접수했어요');
...
// 카풀 드라이버에게 개인 알림 (드라이버는 이름으로 저장됨 → 명단에서 id 매칭)
async function notifyDriver(ride, title, body){
...
document.addEventListener('click', e => {
  const p = document.getElementById('bellPanel');
  if (p && !p.classList.contains('hidden') && !e.target.closest('#bellPanel') && !e.target.closest('.bell-btn')) p.classList.add('hidden');
});
...
async function renderMore() {
  ...
  let _prefs = {}; if (_pushOn) { try { _prefs = await getPushPrefs(); } catch(e){} }
  const _catRows = PUSH_CATS.map(([k,nm,de])=>`
    <label style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:8px 0;border-top:1px solid var(--line);cursor:pointer">
      <span style="min-width:0"><span style="font-size:14px;font-weight:600;color:var(--cream)">${nm}</span><span style="display:block;font-size:11px;color:var(--muted)">${de}</span></span>
      <input type="checkbox" style="width:18px;height:18px;flex-shrink:0;accent-color:var(--accent)" ${_prefs[k]===false?'':'checked'} onchange="setPushPref('${k}', this.checked)">
    </label>`).join('');
    ...
    ? (_pushOn
      ? `<details class="more-item" style="cursor:default" ${_notifOpen?'open':''} ontoggle="_notifOpen=this.open">
          ...
          <div style="margin-top:8px">
            <div class="mi-desc" style="margin-bottom:6px">마감 임박·내일 세션·개인 알림은 나에게 해당할 때만, 항상 와요</div>${_catRows}
...
async function renderHome() {
  ...
  const myConfd = isDuesConfirmed(dMonth, me);   // 총무 입금확인 여부
  ...
  if (meActive) {
    ...
    const capActOn = capApplies ? (myCapSt==='kept'||myCapSt==='queue') : !dormStatus;
...
async function renderSquad() {
  ...
  if (((_lgSq.white||[]).length + (_lgSq.black||[]).length) > 0) {
    ...
    // 감독은 각 팀 맨 위 + 감독 뱃지 (2026-08-31 총괄)
    const chipT = p => _caps.includes(p.id)
      ? chip(p).replace('<span class="sq-dot"', '<span class="win-badge" style="flex-shrink:0;background:var(--gold);color:#23180a">감독</span><span class="sq-dot"')
      : chip(p);
    ...
    const col = (title, sw, arr) => `<div style="flex:1;min-width:0">
        ...
        <div style="display:flex;flex-direction:column;gap:8px">${arr.map(chipT).join('')}</div>
      </div>`;
...
const BADGE_DEFS = [
  ['summer','여름','여름 시즌(7~8월) 3회 이상 참여',
   `<circle cx="24" cy="24" r="22" fill="#FFE8B0"/><circle cx="15.5" cy="13.5" r="7" fill="#F7C948" opacity=".35"/><circle cx="15.5" cy="13.5" r="4.8" fill="#F0A34B"/><circle cx="14.2" cy="12.2" r="1.3" fill="#FBE3A9"/><path d="M31.5 13.5 q2 -2.2 4 0 M35.5 13.5 q2 -2.2 4 0" fill="none" stroke="#9C8767" stroke-width="1.4" stroke-linecap="round"/><path d="M3.15 31 Q9 29 14 30.8 Q19 32.6 24 30.8 Q29 29 34 30.8 Q39 32.6 44.85 31 A 22 22 0 0 1 3.15 31 Z" fill="#6FC3EA"/><path d="M9 35.5 q3 -2.2 6 0 M21 38 q3 -2.2 6 0 M31 34.5 q3 -2.2 6 0" fill="none" stroke="#C9E9F8" stroke-width="1.8" stroke-linecap="round"/>`],
  ['winter','겨울','겨울 시즌(전년 12월~1월) 3회 이상 참여',
   `<circle cx="24" cy="24" r="22" fill="#D3E5F5"/><path d="M4 33 Q13 26.5 24 31 Q35 26.5 44 33 A 22 22 0 0 1 4 33 Z" fill="#FFFFFF"/><path d="M14 19.5 l4.6 9.5 h-9.2 z" fill="#3F8464"/><path d="M14 24 l3.8 7.5 h-7.6 z" fill="#4E9971"/><rect x="13.1" y="29" width="1.8" height="3" fill="#8A6748"/><g fill="#FFFFFF"><circle cx="22" cy="11" r="1.5"/><circle cx="31" cy="14" r="1.3"/><circle cx="38" cy="21" r="1.4"/><circle cx="27" cy="20" r="1.2"/><circle cx="19" cy="15.5" r="1.1"/><circle cx="34" cy="26" r="1.2"/><circle cx="10" cy="22" r="1.3"/><circle cx="24" cy="26.5" r="1"/></g>`],
  ['league','팀 리그','리그 달(3·5·9·11월) 3회 이상 참여',
   `<circle cx="24" cy="24" r="22" fill="#FCE9AC"/><path d="M15 12.5 h18 v8.5 a9 9 0 0 1 -18 0 z" fill="#D99A2B"/><path d="M15 14.5 h-3.6 a5.2 5.2 0 0 0 5.4 6.2 M33 14.5 h3.6 a5.2 5.2 0 0 1 -5.4 6.2" fill="none" stroke="#D99A2B" stroke-width="2.2"/><path d="M18.5 14.5 q0 6.5 3 9.5 l0 -0.5 q-2.2 -3.5 -1.6 -9 z" fill="#F1C463"/><rect x="21.8" y="29.5" width="4.4" height="4" fill="#C9891F"/><rect x="16" y="33.5" width="16" height="4" rx="1.6" fill="#B47A1C"/>`],
  ['soccer','축구','축구 경기 참여',
   `<defs><clipPath id="scfBallClip"><circle cx="24" cy="24" r="15"/></clipPath></defs><circle cx="24" cy="24" r="22" fill="#D9EEDC"/><circle cx="24" cy="24" r="15" fill="#fff"/><g clip-path="url(#scfBallClip)"><g stroke="#2A3440" stroke-width="1.5"><line x1="24" y1="18.4" x2="24" y2="9"/><line x1="29.3" y1="22.3" x2="38.3" y2="19.4"/><line x1="27.3" y1="28.5" x2="32.8" y2="36.1"/><line x1="20.7" y1="28.5" x2="15.2" y2="36.1"/><line x1="18.7" y1="22.3" x2="9.7" y2="19.4"/></g><path d="M 24 7.5 L 27.7 10.2 L 26.3 14.6 L 21.7 14.6 L 20.3 10.2 Z" fill="#2A3440"/><path d="M 39.7 18.9 L 38.3 23.3 L 33.7 23.3 L 32.3 18.9 L 36 16.2 Z" fill="#2A3440"/><path d="M 33.7 37.3 L 29.1 37.3 L 27.7 33 L 31.4 30.3 L 35.1 33 Z" fill="#2A3440"/><path d="M 14.3 37.3 L 12.9 33 L 16.6 30.3 L 20.3 33 L 18.9 37.3 Z" fill="#2A3440"/><path d="M 8.3 18.9 L 12 16.2 L 15.7 18.9 L 14.3 23.3 L 9.7 23.3 Z" fill="#2A3440"/><path d="M 24 18.4 L 29.3 22.3 L 27.3 28.5 L 20.7 28.5 L 18.7 22.3 Z" fill="#2A3440"/></g><circle cx="24" cy="24" r="15" fill="none" stroke="#2A3440" stroke-width="1.8"/>`],
  ['dinner','회식','회식 참여',
   `<circle cx="24" cy="24" r="22" fill="#FCE3C0"/><g transform="rotate(-9 17.5 27)"><rect x="12.5" y="19.5" width="9.5" height="13.5" rx="2" fill="#E39A3B"/><rect x="12.5" y="19.5" width="9.5" height="4.6" rx="2" fill="#FFF6E6"/><rect x="14.3" y="25" width="1.6" height="6" rx="0.8" fill="#F1BC72"/></g><g transform="rotate(9 30.5 27)"><rect x="26" y="19.5" width="9.5" height="13.5" rx="2" fill="#CE8226"/><rect x="26" y="19.5" width="9.5" height="4.6" rx="2" fill="#FFF6E6"/><rect x="27.8" y="25" width="1.6" height="6" rx="0.8" fill="#E0A653"/></g><g fill="#F1C463"><circle cx="24" cy="12.5" r="1.5"/><circle cx="19.5" cy="15" r="1.1"/><circle cx="28.5" cy="15" r="1.1"/></g>`],
  ['outing','야유회','싸커피 여름 야유회 참여',
   `<circle cx="24" cy="24" r="22" fill="#CDEDE7"/><path d="M4.4 34 Q24 26.5 43.6 34 A 22 22 0 0 1 4.4 34 Z" fill="#F3DCA8"/><path d="M12 22 a14 14 0 0 1 24 0 z" fill="#E8795A"/><path d="M17 22 q3.5 -9 7 -11 q3.5 2 7 11 z" fill="#F7CDBB"/><line x1="24" y1="11" x2="24" y2="36" stroke="#8A6748" stroke-width="2.2" stroke-linecap="round"/><circle cx="24" cy="9.8" r="1.7" fill="#8A6748"/><circle cx="33.5" cy="33.5" r="3" fill="#6FC3EA"/><path d="M33.5 30.5 a3 3 0 0 1 0 6" fill="#FFFFFF" opacity=".85"/>`],
  ['day','싸커피 데이','싸커피 데이 참여',
   `<circle cx="24" cy="24" r="22" fill="#EFE5D2"/><path d="M14 19 h17 v10 a7 7 0 0 1 -7 7 h-3 a7 7 0 0 1 -7 -7 z" fill="#46331F"/><path d="M31 21.5 h3.2 a3.6 3.6 0 0 1 0 7.2 h-3.6" fill="none" stroke="#46331F" stroke-width="2.2"/><path d="M19 15.5 q-1.6 -2 0 -3.8 M24.5 15.5 q-1.6 -2 0 -3.8" fill="none" stroke="#A98F6B" stroke-width="1.8" stroke-linecap="round"/><path d="M36.5 10.5 l1.2 2.6 2.6 1.2 -2.6 1.2 -1.2 2.6 -1.2 -2.6 -2.6 -1.2 2.6 -1.2 z" fill="#D9A441"/>`],
  ['driver','드라이버','카풀 운전 5회 이상',
   `<circle cx="24" cy="24" r="22" fill="#E6E3F8"/><path d="M11.5 27 l3 -7 a3.2 3.2 0 0 1 3 -2 h13 a3.2 3.2 0 0 1 3 2 l3 7 z" fill="#7B6FD0"/><rect x="9" y="26" width="30" height="7.5" rx="2.8" fill="#8A7FDB"/><path d="M15.6 19.6 h7 v6 h-9.2 z" fill="#EFEDFB"/><path d="M24.4 19.6 h7.4 l2.4 6 h-9.8 z" fill="#EFEDFB"/><circle cx="19" cy="23.2" r="2.1" fill="#8A6F4D"/><circle cx="28.6" cy="23.2" r="2.1" fill="#B98A5A"/><circle cx="16.5" cy="34.5" r="3.2" fill="#35304F"/><circle cx="31.5" cy="34.5" r="3.2" fill="#35304F"/><circle cx="16.5" cy="34.5" r="1.3" fill="#fff"/><circle cx="31.5" cy="34.5" r="1.3" fill="#fff"/><circle cx="11.8" cy="29.8" r="1.3" fill="#F1C463"/><circle cx="36.2" cy="29.8" r="1.3" fill="#F1C463"/>`]
];
...
async function memberStats(id, joinDate){
  ...
  try {
    ...
    if (tp) {
      ...
      try { st.badges = await badgeComputeFull(tp, tb); } catch(e) {}
    }
...
async function renderDues() {
  ...
  const myCard = meActive ? `<div class="card" style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 16px;margin-bottom:12px">
      <div style="min-width:0">
        ...
        <div style="font-size:12px;color:var(--muted);margin-top:2px">${myPaid ? (myConfd ? '<b style="color:var(--win)">입금 확인됨</b>' : '납부 표시됨 · 확인 대기중') : '미납 상태예요'}${myDirty?' · 저장 전 (아래 저장 필요)':''}</div>
      </div>
      ${admin
