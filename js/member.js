  ...
  if (!isVotingOpen()) {
    ...
    el.innerHTML = `
      <div class="potm-hero">
        ...
        <div class="turnout">${parseInt(month.slice(5))}월 투표는 <b>15일</b>부터 열려요</div>
      </div>
...
function voterOptions(members) {
  return [...members].sort(byJersey)
    .map(m => `<option value="${m.id}">${m.jersey != null ? m.jersey + ' · ' : ''}${esc(m.name)}</option>`).join('');
}
...
function candSelect(candidates, cat) {
  const sorted = [...candidates].sort(byJersey);
...
function renderFaq() {
  ...
  const rules = [
    ...
    ['휴면', ['한 달 단위로 활동 쉬기', '매월 <b>15일</b>부터 홈에서 다음 달 신청', '휴면 달은 회비 · 참석 · 명단 · 투표 제외']],
...
async function renderAtt() {
  ...
  const sortedM = [...members].sort(byJersey);
...
async function renderDues() {
  ...
  const transCard = (toActive.length || toDormant.length) ? `<div class="card" style="padding:12px 14px;margin-bottom:12px">
      ...
      ${toActive.length?`<div style="font-size:13px;line-height:1.7${toDormant.length?';margin-bottom:6px':''}"><span style="color:var(--win);font-weight:800">휴면→활동</span> <span style="color:var(--muted)">${toActive.length}명</span> · ${[...toActive].sort(byJersey).map(m=>esc(m.name)).join(', ')}</div>`:''}
      ${toDormant.length?`<div style="font-size:13px;line-height:1.7"><span style="color:var(--alert);font-weight:800">활동→휴면</span> <span style="color:var(--muted)">${toDormant.length}명</span> · ${[...toDormant].sort(byJersey).map(m=>esc(m.name)).join(', ')}</div>`:''}
    </div>` : '';
  ...
  let html = `
    ...
    ${(total-paidCount)>0 ? `<div class="card" style="padding:12px 14px;margin-bottom:12px">
      ...
      <div style="font-size:14px;line-height:1.7;color:var(--text)">${[...payMembers].filter(m=>effState(m.id)==='unpaid').sort(byJersey).map(m=>esc(m.name)).join(', ')}</div>
    </div>` : ''}
