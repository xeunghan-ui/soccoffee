...
// 알림 종류(멤버 선택) — 기본 전부 켜짐. 개인 알림·수동 발송·휴면 복귀 확인은 항상 감
const PUSH_CATS = [
  ['news',    '공지',       '새 공지'],
  ['session', '세션',       '새 일정 · 내일 세션 · 마감 임박'],
  ['ride',    '카풀',       '새 카풀 등록'],
  ['dues',    '회비',       '납부 시작 · 마감 임박'],
  ['vote',    '투표',       '이달의 선수 투표 시작'],
];
...
async function renderMore() {
  ...
  const notifBtn = PUSH_SUPPORTED || IS_IOS
    ? (_pushOn
      ? `<div class="more-item" style="cursor:default"><div class="mi-name">알림 <span style="font-size:11px;color:var(--win);font-weight:800">켜짐</span></div><div class="mi-desc" style="margin-bottom:6px">받고 싶은 알림을 고르세요 · 입금확인 등 개인 알림은 항상 와요</div>${_catRows}
        <button class="btn ghost sm" style="margin-top:10px;width:100%" onclick="disablePush()">알림 전체 끄기</button></div>`
